...
const ProfilePage: React.FC = () => {
  ...
  return (
    <div className="max-w-5xl space-y-6 animate-fade-in">
      ...
      {/* User info card */}
      <div className="flex items-center gap-4 rounded-2xl bg-white p-5 shadow-summary dark:bg-slate-900">
        <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-cyan-400 to-sky-500 text-2xl font-bold text-slate-950 shadow-lg shadow-cyan-500/20">
          {user?.fullName ? user.fullName[0].toUpperCase() : <User className="size-6" />}
        </div>
        <div>
          ...
          <div className="mt-1 flex flex-wrap items-center gap-3 text-slate-500 dark:text-slate-400">
            <span className="flex items-center gap-1"><Mail className="size-icon-caption" /> {profile?.email || user?.email}</span>
            <span className="text-slate-300 dark:text-slate-600">|</span>
            <span className="flex items-center gap-1">
              <Calendar className="size-icon-caption" />
          ...
          {(user?.role === 'ADMIN' || profile?.role === 'ADMIN') && (
            <span className="inline-flex items-center gap-1 mt-2 app-badge-warning">
              <Shield className="size-icon-tiny" /> Quản trị viên
            </span>
          )}
        </div>
      </div>
      ...
      <div className="rounded-2xl bg-white p-6 shadow-summary dark:bg-slate-900">
        <div className="mb-6 flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-blue-100 dark:bg-brand-500/10">
            <User  className="size-icon-large text-brand-500" />
          </div>
          <div>
      ...
      <div className="rounded-2xl bg-white p-6 shadow-summary dark:bg-slate-900">
        <div className="mb-6 flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-rose-100 dark:bg-rose-500/10">
            <Lock  className="size-icon-large text-rose-500" />
          </div>
          <div>
        ...
        <form onSubmit={handlePasswordSubmit} className="space-y-4">
          ...
          >
            {changePasswordMutation.isPending ? <Loader2  className="size-4 animate-spin" /> : <Lock className="size-4" />}
            Đổi mật khẩu
          </AppButton>
        </form>
      </div>
