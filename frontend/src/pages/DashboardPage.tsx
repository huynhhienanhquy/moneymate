...
const StatCard: React.FC<{
  ...
  <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex items-start gap-4 hover:border-slate-700 transition-colors">
    ...
    <div className="flex-1 min-w-0">
      ...
      <p className="mt-0.5 text-xl font-bold text-slate-100 truncate">{value}</p>
  ...
  return (
    <div className="space-y-8">
      ...
      {aiInsight?.insights?.[0] && (
        <Link to="/ai" className="flex items-center gap-4 p-4 bg-brand-600/10 border border-brand-500/20 rounded-2xl hover:bg-brand-600/15 transition group">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-slate-400 truncate">{aiInsight.insights[0].message}</p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-2xl p-6">
          <h2 className="text-base font-semibold text-slate-200 mb-5">Chi tiêu theo danh mục</h2>
              ...
              <ul className="mt-2 space-y-2">
                {categoryExpenses.slice(0, 5).map((cat, i) => (
                  <li key={cat.id} className="flex items-center justify-between text-xs">
                    ...
                    <span className="font-semibold text-slate-200">{formatVND(cat.amount)}</span>
                  </li>
                ))}
              </ul>
            </>
        ...
        <div className="lg:col-span-3 bg-slate-900 border border-slate-800 rounded-2xl p-6">
          <h2 className="text-base font-semibold text-slate-200 mb-5">Thu nhập & Chi tiêu 6 tháng</h2>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
        <h2 className="text-base font-semibold text-slate-200 mb-5">Giao dịch gần đây</h2>
        {recentTransactions.length > 0 ? (
          <div className="space-y-3">
            {recentTransactions.map((tx: any) => {
              const isIncome = tx.type === 'INCOME';
              return (
                <div key={tx.id} className="flex items-center gap-4 p-3 rounded-xl hover:bg-slate-800/60 transition-colors group">
                  <div
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-slate-200 truncate">{tx.note || tx.category?.name}</p>
