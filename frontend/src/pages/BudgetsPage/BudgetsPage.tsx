...
const ProgressRing = ({ percentage, tone }: { percentage: number; tone: 'safe' | 'warning' | 'danger' }) => {
  ...
  const stroke = tone === 'danger' ? chartTheme.expense : tone === 'warning' ? chartTheme.warning : chartTheme.income;
  const radius = chartTheme.ringRadius;
  ...
  return (
    <div className="relative h-24 w-24 shrink-0">
      ...
      <span className="absolute inset-0 flex items-center justify-center text-lg font-extrabold text-slate-800 dark:text-slate-100">{Math.round(progress)}%</span>
    </div>
  );
};
  ...
  return (
    <AppModal onClose={onClose}>
        ...
        <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 mb-4 bg-slate-50 dark:bg-slate-800/60 rounded-xl px-4 py-2.5">
          <PiggyBank className="size-3.5" /> Tháng {MONTHS[month - 1]} {year}
        </div>
              ...
              <div key={b.id} className={`group relative overflow-hidden rounded-xl border-t-4 bg-white p-4 shadow-category dark:bg-slate-900 ${isExceeded ? 'border-rose-500' : isWarning ? 'border-amber-400' : 'border-emerald-400'}`}>
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition">
                    <AppButton unstyled onClick={() => setEditBudget(b)} className="p-1.5 rounded-lg text-slate-500 hover:text-slate-200 hover:bg-slate-800 transition"><Pencil className="size-3.5" /></AppButton>
                ...
                {fc && fc.severity !== 'OK' && (
                  <p className="text-xs mt-2 text-amber-500 dark:text-amber-400/80 border-t border-slate-100 dark:border-slate-800 pt-2 flex items-center gap-1">
                    <AlertTriangle className="size-icon-tiny" /> {fc.message}
                  </p>
                )}
              </div>
            )})}
