  ...
  return (
    <AuthShell titleId="register-title" register>
      ...
      <form onSubmit={submit} className="auth-form">
        ...
        <AuthField id="register-password" name="password" label="Mật khẩu" icon={<LockKeyhole size={19} />} type={showPassword ? 'text' : 'password'} autoComplete="new-password" value={form.password} onChange={change} placeholder="Tối thiểu 8 ký tự" trailing={<button type="button" className="auth-eye" aria-label={showPassword ? 'Ẩn mật khẩu' : 'Hiện mật khẩu'} onClick={() => setShowPassword(!showPassword)}>{showPassword ? <EyeOff size={19} /> : <Eye size={19} />}</button>} />
        ...
        <button id="register-submit" type="submit" disabled={loading || success} className="auth-submit">{loading ? <Loader2 size={18} className="animate-spin" /> : <>Đăng ký <ArrowRight size={18} /></>}</button>
      </form>
