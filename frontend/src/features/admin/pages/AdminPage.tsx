  ...
  return (
    <div className="min-h-screen border-t border-[#665cff] bg-[#f7f9fb] text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      <div className="flex h-9 items-center justify-end gap-5 border-b border-slate-200 bg-white px-3 text-slate-500 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400">
        <button type="button" aria-label="Thông báo" className="transition hover:text-slate-900 dark:hover:text-white"><Bell size={14} /></button>
        <button type="button" aria-label="Trợ giúp" className="transition hover:text-slate-900 dark:hover:text-white"><CircleHelp size={14} /></button>
      ...
      <header className="flex items-end justify-between gap-4">
        ...
        <button type="button" onClick={handleLogout} className="inline-flex h-7 items-center gap-1.5 rounded-md border border-rose-400 bg-white px-3 text-[10px] font-semibold text-rose-500 transition hover:bg-rose-50 dark:border-rose-500/40 dark:bg-slate-900 dark:hover:bg-rose-500/10">
      ...
      <div className="mt-5 flex h-[42px] items-center justify-between gap-4 rounded-md bg-white px-2.5 shadow-[0_4px_18px_rgba(15,23,42,0.07)] dark:bg-slate-900">
        <label className="relative block w-[210px] sm:w-[310px]">
          ...
          {search && <button type="button" aria-label="Xóa từ khóa tìm kiếm" onClick={() => setSearch('')} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-700 dark:hover:text-slate-200"><X size={15} /></button>}
        </label>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {pageUsers.map((user: any) => (
                  <tr key={user.id} className="group transition-colors hover:bg-blue-50/40 dark:hover:bg-slate-800/40">
                    <td className="px-3 py-2.5"><button type="button" onClick={() => setEditUser(user)} className="flex items-center gap-2 text-left" title="Chỉnh sửa tài khoản"><span className="flex h-[22px] w-[22px] shrink-0 items-center justify-center rounded-full bg-[#062b55] text-[9px] font-extrabold text-white">{user.fullName?.[0]?.toUpperCase() || '?'}</span><span className="line-clamp-3 text-[8px] font-semibold leading-[9px] text-slate-950 group-hover:text-blue-700 dark:text-slate-100">{user.fullName}{user.role === 'ADMIN' && <ShieldCheck size={8} className="ml-0.5 inline text-blue-600" />}</span></button></td>
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-slate-700 dark:bg-slate-900">
        <div className="mb-6 flex items-center justify-between"><div className="flex items-center gap-2.5"><span className="rounded-lg bg-blue-50 p-2 text-blue-600 dark:bg-blue-500/15 dark:text-blue-400"><UserCog size={19} /></span><h2 className="text-lg font-bold text-slate-950 dark:text-white">Cập nhật tài khoản</h2></div><button type="button" aria-label="Đóng" onClick={onClose} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-slate-800 dark:hover:text-slate-200"><X size={18} /></button></div>
        ...
        <div className="mt-6 flex items-center gap-3"><button type="button" onClick={onDelete} disabled={loading} aria-label="Xóa người dùng" className="flex h-10 w-10 items-center justify-center rounded-lg border border-rose-200 text-rose-500 transition hover:bg-rose-50 disabled:opacity-50 dark:border-rose-500/30 dark:hover:bg-rose-500/10"><Trash2 size={17} /></button><button type="button" onClick={onClose} className="h-10 flex-1 rounded-lg border border-slate-200 text-sm font-semibold text-slate-600 transition hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800">Hủy</button><button type="button" onClick={() => onSave(form)} disabled={loading || !form.fullName.trim()} className="flex h-10 flex-1 items-center justify-center rounded-lg bg-blue-600 text-sm font-semibold text-white transition hover:bg-blue-700 disabled:opacity-50">{loading ? <Loader2 size={16} className="animate-spin" /> : 'Lưu thay đổi'}</button></div>
      </div>
    </div>
  );
};
