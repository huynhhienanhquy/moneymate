    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Phân tích thu chi theo tháng và năm</p>
        </div>
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <div className="flex gap-2 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl p-1 shadow-sm">
      ...
      <div className="flex items-center justify-center gap-4">
        <button onClick={prevPeriod} className="p-2 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition">
          <ChevronLeft size={18} />
        </button>
        <div className="flex items-center gap-2 text-slate-900 dark:text-slate-200 font-semibold app-card px-4 py-2">
          <Calendar size={18} className="text-brand-400" />
          {reportType === 'monthly' ? `${MONTHS[month - 1]} ${year}` : `Năm ${year}`}
        </div>
        <button onClick={nextPeriod} className="p-2 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition">
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {/* Pie Chart */}
            <div className="app-card p-6">
              <h2 className="text-base font-bold text-slate-900 dark:text-slate-100 mb-5">Top chi tiêu theo danh mục</h2>
                    ...
                    {categoryExpenses.slice(0, 8).map((cat: any, i: number) => {
                      ...
                      return (
                        <li key={cat.id} className="flex items-center justify-between text-xs">
                          ...
                          <span className="font-semibold text-slate-900 dark:text-slate-200">{formatVND(cat.amount)} ({pct}%)</span>
                        </li>
                      );
                    })}
                  </ul>
