...
export default function WalletsPage() {
  ...
  const save = useMutation({ mutationFn: () => apiRequest(editing ? `/wallets/${editing.id}` : '/wallets', { method: editing ? 'PUT' : 'POST', body: JSON.stringify(editing ? { name: name.trim(), type } : { name: name.trim(), type, initialBalance: Number(balance) }) }), onSuccess: async () => { await client.invalidateQueries({ queryKey: ['wallets'] }); setOpen(false); }, onError: (e) => Alert.alert('Không thể lưu ví', e instanceof Error ? e.message : 'Vui lòng thử lại') });
  const remove = useMutation({
    mutationFn: (id: string) => apiRequest(`/wallets/${id}`, { method: 'DELETE' }),
    onSuccess: () => {
      for (const queryKey of ['wallets', 'dashboard', 'monthly-report', 'monthly-trend', 'trend']) {
        client.invalidateQueries({ queryKey: [queryKey] });
      }
    },
    onError: (e) => Alert.alert('Không thể xóa ví', e instanceof Error ? e.message : 'Vui lòng thử lại'),
  });
  ...
  return <Screen title="Ví tài khoản" action={<Pressable accessibilityRole="button" accessibilityLabel="Thêm ví" onPress={showCreate}><MaterialCommunityIcons name="plus-circle" size={30} color={theme.colors.primary} /></Pressable>}>
    ...
    {wallets.data?.map((wallet) => <EntityCard key={wallet.id} icon={<IconTile name="wallet-outline" />} title={wallet.name} subtitle={wallet.type} value={money(wallet.initialBalance)} badge={<Badge label={wallet.currency || 'VND'} />}><View style={ui.row}><View style={{ flex: 1 }}><ActionLink label="Chỉnh sửa" icon="pencil-outline" onPress={() => showEdit(wallet)} /></View><View style={{ flex: 1 }}><ActionLink danger label="Xóa" icon="trash-can-outline" onPress={() => Alert.alert('Xóa ví?', 'Ví sẽ không còn trong tổng tài sản. Lịch sử giao dịch được giữ và giao dịch định kỳ của ví sẽ dừng.', [{ text: 'Hủy' }, { text: 'Xóa', style: 'destructive', onPress: () => remove.mutate(wallet.id) }])} /></View></View></EntityCard>)}
    <Sheet visible={open} title={editing ? 'Chỉnh sửa ví' : 'Tạo ví mới'} onClose={() => setOpen(false)}><Field label="Tên ví" value={name} onChangeText={setName} /><Text style={ui.muted}>Loại ví</Text><ChoiceChips value={type} options={walletTypes} onChange={setType} />{!editing && <Field label="Số dư" value={balance} onChangeText={setBalance} keyboardType="numeric" />}<Button label={editing ? 'Lưu thay đổi' : 'Tạo ví'} loading={save.isPending} disabled={!name.trim() || (!editing && Number(balance) < 0)} onPress={() => save.mutate()} /></Sheet>
  </Screen>;
}
