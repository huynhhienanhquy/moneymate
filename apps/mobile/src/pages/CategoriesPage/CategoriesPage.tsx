...
export default function CategoriesPage() {
  ...
  const client = useQueryClient(); const [open, setOpen] = useState(false); const [editing, setEditing] = useState<Category | null>(null); const [name, setName] = useState(''); const [type, setType] = useState('EXPENSE'); const [color, setColor] = useState<string>(theme.colors.primary);
  ...
  const start = (item?: Category) => { setEditing(item || null); setName(item?.name || ''); setType(item?.type || 'EXPENSE'); setColor(item?.color || theme.colors.primary); setOpen(true); };
